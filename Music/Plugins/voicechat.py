...
async def activevc(_, message: Message):
    ...
    for x in served_chats:
        ...
        j += 1
    if not text:
        await message.reply_text("**No Active Voice Chat**")
    else:
...
async def basffy(_, message):
    if len(message.command) != 2:
        await message.reply_text(
            "**Penggunaan:**\n/joinassistant [Username Or User Id]"
        )
        return
...
async def baaaf(_, message):
    ...
    try:
        ...
        print(e)
        return
    await message.reply_text("**The Bot Has Successfully Left The Chat**")
...
async def baujaf(_, message):
    ...
    try:
        ...
        await message.reply_text(f"Failed\n**Reason**:{e}")
        return
    await message.reply_text("**Successfully Leave**")
